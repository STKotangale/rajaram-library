...
const PurchhaseReturn = () => {
    ...
    //         purchaseCopyNo: rowData.purchaseCopyNo,
    //     });
    ...
    const addRowEdit = () => {
        const newBookRow = { bookName: '', bookDetails: '' };
        setEditData({ ...editData, bookRows: [...editData.bookRows, newBookRow] });
    };
    
    const deleteRowEdit = (index) => {
        const filteredBookRows = editData.bookRows.filter((_, idx) => idx !== index);
        setEditData({ ...editData, bookRows: filteredBookRows });
    };
    

    const handleEditClick = (issue) => {
        console.log("Row data:", issue);
        setEditData({
            ...issue,
            bookRows: issue.purchaseDetails || []
        });
        setShowEditModal(true);
    };
    ...
    const handleBookChangeForRow = (index, event) => {
        const value = event.target.value;
        const updatedRows = [...rows];
        updatedRows[index] = { ...updatedRows[index], bookName: value, bookDetails: '' };
        setRows(updatedRows);
    };
    ...
    const handleBookChangeInEdit = (index, event) => {
        const updatedBookRows = editData.bookRows.map((row, idx) => {
            if (idx === index) {
                return { ...row, bookName: event.target.value };
            }
            return row;
        });
        setEditData({ ...editData, bookRows: updatedBookRows });
    };
    
    const handleBookDetailsChangeInEdit = (index, event) => {
        const updatedBookRows = editData.bookRows.map((row, idx) => {
            if (idx === index) {
                return { ...row, bookDetails: event.target.value };
            }
            return row;
        });
        setEditData({ ...editData, bookRows: updatedBookRows });
    ...
    return (
        <div className="main-content">
                        ...
                        <Table striped bordered hover className='mt-4'>
                            <thead>
                                <tr>
                                    <th>Sr. No.</th>
                                    <th>Member Name</th>
                                    <th>Purchhase Return No</th>
                                    <th>Purchhase Return Date</th>
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
            ...
            <Modal centered show={showAddModal} onHide={() => setShowAddModal(false)} size='xl'>
                <div className="bg-light">
                    <Modal.Header closeButton>
                        <Modal.Title>Add Purchhase Return</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Form onSubmit={handleSubmit}>
                            <Row className="mb-3">
                                <Form.Group as={Col}>
                                    <Form.Label>Purchhase Return No</Form.Label>
                                    <Form.Control
                                        placeholder="Purchhase return number"
                                        type="text"
                                ...
                                <Form.Group as={Col}>
                                    <Form.Label>Purchhase Return Date</Form.Label>
                                    <Form.Control
                                        type="date"
                                        value={invoiceDate}
            ...
            <Modal centered show={showEditModal} onHide={handleCloseEditModal} size='xl'>
                <div className="bg-light">
                    <Modal.Header closeButton>
                        <Modal.Title>Edit Purchhase Return</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                                            ...
                                            <tr key={index}>
                                                <td>
                                                    <Form.Control
                                                        as="select"
                                                        value={row.bookName}
                                                        onChange={(e) => handleBookChangeInEdit(index, e)}
                                                    >
                                                        <option value="">Select a book</option>
                                                        {bookName.map((book) => (
                                                            <option key={book.bookId} value={book.bookName}>{book.bookName}</option>
                                                ...
                                                <td>
                                                    <Form.Control
                                                        as="select"
                                                        value={row.bookDetails}
                                                        onChange={(e) => handleBookDetailsChangeInEdit(index, e)}
                                                    >
                                                        <option value="">Select Book Details</option>
                                                        {bookDetails
                                                            .filter((detail) => detail.bookName === row.bookName)
                                                            .map((detail) => (
                                                                <option key={detail.bookDetailId} value={detail.purchaseCopyNo}>
            ...
            <Modal centered show={showViewModal} onHide={handleCloseViewModal} size='xl'>
                <div className="bg-light">
                    <Modal.Header closeButton>
                        <Modal.Title>View Purchhase Return</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Form>
                            <Row className="mb-3">
            ...
            <Modal show={showDeleteModal} onHide={handleCancelDelete}>
                ...
                <Modal.Body>
                    Are you sure you want to delete this purchhase return?
                </Modal.Body>
                <Modal.Footer>
